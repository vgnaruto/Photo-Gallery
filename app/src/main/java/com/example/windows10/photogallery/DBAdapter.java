...
public class DBAdapter {
    ...
    public long insertData(DataImage data){
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(DBHelper.COLUMN_2,data.getPath());
        cv.put(DBHelper.COLUMN_3,data.getJudul());
        cv.put(DBHelper.COLUMN_4,data.getNote());
        long id = db.insert(DBHelper.TB_NAME, null, cv);
        return id;
    }
    ...
    public int update(DataImage oldData, DataImage newData){
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(DBHelper.COLUMN_2,newData.getPath());
        cv.put(DBHelper.COLUMN_3,newData.getJudul());
        cv.put(DBHelper.COLUMN_4,newData.getNote());
        String[] args = {oldData.getPath()};
        int count = db.update(DBHelper.TB_NAME, cv, DBHelper.COLUMN_2+" = ?",args);
        return count;
    }
    ...
    public ArrayList<DataImage> read(){
        ...
        ArrayList<DataImage> allFoto = new ArrayList<>();
        while(cursor.moveToNext()){
            String path = cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_2));
            String judul = cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_3));
            String note = cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_4));
            DataImage currentData = new DataImage(path,judul,note);
            allFoto.add(currentData);
        }
        cursor.close();
        return  allFoto;
    ...
    static class DBHelper extends SQLiteOpenHelper {
        public static final String TB_NAME = "gallery";
        public static final String COLUMN_1 = "id";
        public static final String COLUMN_2 = "image_path";
        public static final String COLUMN_3 = "judul";
